...
const App: React.FC = () => {
  ...
  const handleBulkImport = (newChurches: Omit<ChurchLeader, 'id' | 'status'>[]) => {
    ...
    if (user) {
      ...
      batch.commit().catch(err => handleFirestoreError(err, OperationType.WRITE, 'bulk-import'));
    }
    
    setCurrentView('meetings');
    alert(`¡Éxito! Se han cargado ${formatted.length} líderes y organizado en ${newMeetings.length} grupos.`);
  };
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'capacitacion': return (
        <div className="w-full flex flex-col items-center justify-center h-[calc(100vh-14rem)] min-h-[500px] animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="mt-8 px-4 py-6 bg-slate-50 rounded-[2rem] border border-slate-100 mx-2">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-slate-500">Líderes</span>
                  <span className="text-[10px] font-black text-blue-600">{churches.length}</span>
                </div>
                <div className="flex items-center justify-between">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em]">
                  {user ? `Sincronizado Cloud: ${user.email}` : `Guardado Local: ${lastSaved}`}
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-6 bg-slate-900/90 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden p-8 text-center animate-in zoom-in-95">
            ...
            <p className="text-slate-500 font-bold text-sm leading-relaxed mb-8">Esta acción eliminará permanentemente todos los líderes y configuraciones guardadas.</p>
